...
namespace PiranhaBasic.Controllers
{
    public class CmsController : Controller
    {
        ...
        [Route("custom")]
        public async Task<IActionResult> Custom(Guid id, bool draft = false)
        {
            var model = await _loader.GetPage<CustomPage>(id, HttpContext.User, draft);

            return View(model);
        }

        [Route("video")]
        public async Task<IActionResult> Video(Guid id, bool draft = false)
        {
            var model = await _loader.GetPage<VideoPage>(id, HttpContext.User, draft);
